...
describe('test-.mjs::root-dirs: Test Suite for root-dirs Files', function() {
    ...
    describe ('test-.js::root-dirs: [Test A] Test Suite for root-dirs in main repo directory', function() {
        ...
        it('[Test A] Test for Git Root using _getGitRoot', function(done) {
            let result = getroot._getGitRoot(__dirname, { baseType: "git", logger: console.log });
            expect(result).to.equal("C:\\Users\\GB\\Documents\\projects\\requireurl\\get-root\\.git");
            done();
        });

        it('[Test A] Test for Git Root using getGitRoot', function(done) {
            let result = getroot.getGitRoot(__dirname, { baseType: "git", logger: console.log });
            expect(result).to.equal("C:\\Users\\GB\\Documents\\projects\\requireurl\\get-root\\.git");
            done();
        });
